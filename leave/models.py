...
class CompanyLeave(fridayModel):
...
class AvailableLeave(fridayModel):
...
class LeaveRequest(fridayModel):
    employee_id = models.ForeignKey(
...
class LeaverequestComment(fridayModel):
    """
...
class LeaveAllocationRequest(fridayModel):
    leave_type_id = models.ForeignKey(
...
class LeaveallocationrequestComment(fridayModel):
    """
...
class RestrictLeave(fridayModel):
...
if apps.is_installed("attendance"):
    ...
    class CompensatoryLeaveRequest(fridayModel):
        leave_type_id = models.ForeignKey(
            LeaveType, on_delete=models.PROTECT, verbose_name="Leave type"
        )
        employee_id = models.ForeignKey(
...
class LeaveGeneralSetting(fridayModel):
    """
...
if apps.is_installed("attendance"):
    ...
    class CompensatoryLeaverequestComment(fridayModel):
        """
...
class EmployeePastLeaveRestrict(fridayModel):
...
if apps.is_installed("attendance"):
    ...
    class OverrideLeaveRequests(LeaveRequest):
        ...
        #     """
        #     WorkRecords = get_friday_model_class(
