...
from friday.friday_settings import BIO_DEVICE_THREADS
from friday.settings import TIME_ZONE
...
@install_required
@permission_required("biometric.delete_biometricemployees")
def delete_friday_cosec_user(request, user_id, device_id):
    """
...
def dahua_biometric_attendance_logs(device):
    """
    Retrieves logs from a Dahua biometric device and marks attendance in friday.

    This function fetches biometric logs from the specified device, processes the attendance records,
    and updates the attendance system in friday. If an employee has an active clock-in record,
