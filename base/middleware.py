...
from friday_documents.models import DocumentRequest

CACHE_KEY = "friday_company_models_cache_key"
...
class CompanyMiddleware:
    ...
    def _add_company_filter(self, model, company_id):
        ...
        is_friday_manager = isinstance(model.objects, fridayCompanyManager)
        ...
        if is_company_model:
            ...
            elif is_friday_manager and related_company_field:
                model.add_to_class(
            ...
            for app_label, models in app_model_mappings.items():
                if apps.is_installed(app_label):
                    company_models.extend(
                        [get_friday_model_class(app_label, model) for model in models]
                    )
