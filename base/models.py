...
from base.friday_company_manager import fridayCompanyManager
from friday import friday_middlewares
from friday.friday_middlewares import _thread_locals
from friday.models import fridayModel
from friday_audit.models import fridayAuditInfo, fridayAuditLog
...
class JobPosition(fridayModel):
    ...
    objects = fridayCompanyManager("department_id__company_id")
...
class JobRole(fridayModel):
    ...
    objects = fridayCompanyManager("job_position_id__department_id__company_id")
...
class RotatingWorkType(fridayModel):
    """
    ...
    objects = fridayCompanyManager("employee_id__employee_work_info__company_id")
...
class RotatingWorkTypeAssign(fridayModel):
    """
...
class EmployeeType(fridayModel):
    """
    EmployeeType model
    """
...
class EmployeeShift(fridayModel):
    """
    EmployeeShift model
    """
    ...
    objects = fridayCompanyManager("employee_shift__company_id")
...
class EmployeeShiftSchedule(fridayModel):
    """
    ...
    auto_punch_out_time = models.TimeField(
        ...
        help_text=_(
            "Time at which the friday will automatically check out the employee attendance if they forget."
        ),
    )
    company_id = models.ManyToManyField(Company, blank=True, verbose_name=_("Company"))

    objects = fridayCompanyManager("shift_id__employee_shift__company_id")
...
class RotatingShift(fridayModel):
    """
    RotatingShift model
    """
...
class RotatingShiftAssign(fridayModel):
    """
...
class WorkTypeRequest(fridayModel):
    """
    ...
    def clean(self):
        request = getattr(friday_middlewares._thread_locals, "request", None)
...
class WorkTypeRequestComment(fridayModel):
    """
...
class ShiftRequest(fridayModel):
    """
    ShiftRequest model
    """
...
class ShiftRequestComment(fridayModel):
    """
...
class Tags(fridayModel):
    title = models.CharField(max_length=30)
    color = models.CharField(max_length=30)
    company_id = models.ForeignKey(
        Company, null=True, editable=False, on_delete=models.PROTECT
    )
    objects = fridayCompanyManager(related_company_field="company_id")

    def __str__(self):
        return self.title


class fridayMailTemplate(fridayModel):
...
class DynamicEmailConfiguration(fridayModel):
    """
...
class MultipleApprovalCondition(fridayModel):
...
class Announcement(fridayModel):
    """
...
class AnnouncementComment(fridayModel):
    """
...
class DashboardEmployeeCharts(fridayModel):
    from employee.models import Employee
...
class TrackLateComeEarlyOut(fridayModel):
...
class CompanyLeaves(fridayModel):
...
class PenaltyAccounts(fridayModel):
    """
