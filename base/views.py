...
from friday import friday_apps
...
from friday.friday_settings import (
    APPS,
    DB_INIT_PASSWORD,
...
from friday.methods import get_friday_model_class, remove_dynamic_url
from friday_audit.forms import HistoryTrackingFieldsForm
from friday_audit.models import AccountBlockUnblock, AuditTag, HistoryTrackingFields
...
def initialize_database(request):
    ...
    if initialize_database_condition():
        ...
        return render(request, "initialize_database/friday_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request, "initialize_database/friday_user_signup.html")
        ...
        return render(
            request,
            "initialize_database/friday_company.html",
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/friday_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/friday_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/friday_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/friday_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/friday_job_position.html",
...
class fridayPasswordResetView(PasswordResetView):
    """
    friday View for Reset Password
    """
...
class EmployeePasswordResetView(PasswordResetView):
    """
    friday View for Employee Reset Password
    """
...
def object_delete(request, obj_id, **kwargs):
    ...
    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_friday_model_class(app_label="pms", model="keyresult")
...
def mail_server_test_email(request):
    ...
    white_labelling = getattr(friday_apps, "WHITE_LABELLING", False)
    image_path = path.join(settings.STATIC_ROOT, "images/ui/friday-logo.png")
    company_name = "friday"
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = _("Test mail from friday")
...
@login_required
@hx_request_required
@permission_required("base.change_fridaymailtemplate")
def view_mail_template(request, obj_id):
    """
    This method is used to display the template/form to edit
    """
    template = fridayMailTemplate.objects.get(id=obj_id)
...
@login_required
@require_http_methods(["POST"])
@permission_required("base.add_fridaymailtemplate")
...
@login_required
@permission_required("base.delete_fridaymailtemplate")
def delete_mail_templates(request):
    ids = request.GET.getlist("ids")
    result = fridayMailTemplate.objects.filter(id__in=ids).delete()
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_friday_model_class(app_label="attendance", model="gracetime")
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_friday_model_class(
            app_label="payroll", model="payrollsettings"
        )
        EncashmentGeneralSettings = get_friday_model_class(
...
@login_required
@permission_required("friday_audit.change_accountblockunblock")
...
@login_required
@permission_required("friday_audit.view_audittag")
...
@login_required
@hx_request_required
@permission_required("friday_audit.add_audittag")
...
@login_required
@hx_request_required
@permission_required("friday_audit.change_audittag")
...
@login_required
def get_friday_installed_apps(request):
