...
from friday.filters import fridayPaginator
from friday.group_by import group_by_queryset
from friday.friday_settings import friday_DATE_FORMATS
from friday.methods import get_friday_model_class
from friday_audit.models import AccountBlockUnblock, HistoryTrackingFields
from friday_documents.forms import (
    DocumentForm,
    DocumentRejectForm,
    DocumentRequestForm,
    DocumentUpdateForm,
)
from friday_documents.models import Document, DocumentRequest
...
@login_required
@manager_can_enter("friday_documents.view_documentrequest")
def document_request_view(request):
    ...
    documents = filtersubordinates(
        request=request,
        perm="friday_documents.view_documentrequest",
        queryset=documents,
    )
...
@login_required
@hx_request_required
@manager_can_enter("friday_documents.add_documentrequest")
def document_request_create(request):
    ...
    form = choosesubordinates(request, form, "friday_documents.add_documentrequest")
    if request.method == "POST":
        form = DocumentRequestForm(request.POST)
        form = choosesubordinates(
            request, form, "friday_documents.add_documentrequest"
        )
        if form.is_valid():
            form = form.save()
...
@login_required
@hx_request_required
@manager_can_enter("friday_documents.change_documentrequest")
...
@login_required
@hx_request_required
@owner_can_enter("friday_documents.view_document", Employee)
...
@login_required
@hx_request_required
@owner_can_enter("friday_documents.add_document", Employee)
...
def document_delete(request, id):
    ...
    try:
        ...
        if not request.user.has_perm("friday_documents.delete_document"):
...
@login_required
@hx_request_required
@manager_can_enter("friday_documents.add_document")
...
def paginator_qry(qryset, page_number):
    ...
    """
    paginator = fridayPaginator(qryset, get_pagination())
        ...
        for models in related_models:
            ...
            if field_name:
                ...
                if (
                    ...
                    Recruitment = get_friday_model_class(
                    ...
                    OnboardingStage = get_friday_model_class(
                    ...
                    OnboardingTask = get_friday_model_class(
    ...
    for column_value, column_name in selected_columns:
        ...
        for employee in employees:
            ...
            if type(value) == date:
                ...
                for format_name, format_string in friday_DATE_FORMATS.items():
...
def joining_today_count(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_friday_model_class(app_label="recruitment", model="candidate")
...
def bonus_points_tab(request, emp_id):
    ...
    try:
        ...
        if apps.is_installed("payroll"):
            Reimbursement = get_friday_model_class(
