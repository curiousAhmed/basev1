...
from friday_audit.methods import get_diff
...
class Employee(models.Model):
    ...
    def get_archive_condition(self):
        ...
        if apps.is_installed("onboarding"):
            OnboardingStage = get_friday_model_class("onboarding", "onboardingstage")
            OnboardingTask = get_friday_model_class("onboarding", "onboardingtask")
        ...
        if apps.is_installed("recruitment"):
            Recruitment = get_friday_model_class("recruitment", "recruitment")
            Stage = get_friday_model_class("recruitment", "stage")
    ...
    def check_online(self):
        ...
        """
        if apps.is_installed("attendance"):
            Attendance = get_friday_model_class("attendance", "attendance")
...
class EmployeeBankDetails(fridayModel):
    """
...
class EmployeeNote(fridayModel):
    """
    EmployeeNote model
    """
...
class PolicyMultipleFile(fridayModel):
    """
...
class Policy(fridayModel):
    ...
    objects = fridayCompanyManager("company_id")
...
class DisciplinaryAction(fridayModel):
    """
    Disciplinary model
    """
...
class EmployeeGeneralSetting(fridayModel):
    """
...
class ProfileEditFeature(fridayModel):
    """
    ProfileEditFeature
    """
